...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1033'">
            <xsl:text>Harvard - Ref 9</xsl:text>
          </xsl:when>
	...
	<xsl:template name="templ_prop_NoAndBeforeLastAuthorBackup" >
		<xsl:param name="LCID" />
		<xsl:variable name="_LCID">
			<xsl:call-template name="localLCID">
				<xsl:with-param name="LCID" select="$LCID"/>
	...
	<xsl:template name="templ_prop_BeforeLastAuthorBackup" >
		<xsl:param name="LCID" />
		<xsl:variable name="_LCID">
			<xsl:call-template name="localLCID">
				<xsl:with-param name="LCID" select="$LCID"/>
					...
					<xsl:choose>
						<xsl:when test="$cAuthorFirstName=1">
							...
							<xsl:choose>
								...
								<xsl:when test ="$cAuthorMiddleName=0 and $cAuthors>1 and (position())!=$cAuthors">
									<xsl:call-template name ="templ_prop_ListSeparator"/>
								</xsl:when>
								<xsl:when test ="$cAuthorMiddleName!=0">
									<xsl:call-template name ="templ_prop_Space"/>
								</xsl:when>
								<xsl:when test ="$cAuthors>0 and (position())=$cAuthors">
									<xsl:call-template name ="templ_prop_Space"/>
								</xsl:when>
							</xsl:choose>
						</xsl:when>
					...
					<xsl:choose>
						<xsl:when test="$cAuthorMiddleName=1">
							...
							<xsl:choose>
								...
								<xsl:when test ="$cAuthors>1 and (position())=$cAuthors">
									<xsl:call-template name ="templ_prop_Space"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:call-template name ="templ_prop_Space"/>
	...
	<xsl:template name="BibDisplayTitleJ">
		<xsl:variable name="cTitle">
			<xsl:value-of select="count(b:Title)"/>
		</xsl:variable>
		<xsl:if test ="$cTitle!=0">
			...
			<xsl:choose>
				...
				<xsl:when test="$SourceType = 'JournalArticle'">
					<xsl:element name="p">
						...
						<xsl:call-template name = "BibDisplayYear"/>
						<xsl:call-template name = "BibDisplayTitleJ"/>
						<i>
